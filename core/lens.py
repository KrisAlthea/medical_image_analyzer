...
`methods.py` 文件实现了一个完整的图像处理 lens_process，利用 UNet 模型进行语义分割，并对分割结果进行后处理以提取和优化边界点集，最终在原始图像上可视化边界。整个流程包括模型加载、图像预处理、模型预测、边界点提取、噪声去除、边界拟合和结果可视化。代码结构清晰，功能模块化，易于理解和扩展。
"""

"""
语义分割模型
参数列表：
    img: opencv读取的原始图像，大小为（1024*1440*3）
    best_model_path: 最佳分割模型路径
返回结果：
    pred: unet分割结果，大小为（1024*1440）
"""


def unet_seg_method(image, model_path):
    """
    使用UNet模型对输入图像进行语义分割。

    :param image: 输入的原始图像，opencv读取，形状为(1024, 1440, 3)
    :param model_path: 最佳模型权重文件的路径
    ...
    # 1.2 初始化UNet模型
    net = UNet(in_channels=3, n_classes=2, channels=96)
    # 1.3 加载预训练权重
...
if __name__ == "__main__":
    ...
    # 执行语义分割
    pred = unet_seg_method(img, best_model_path)
    # plt.imshow(pred)
    # plt.title("Segmentation Result")
    # plt.show()

    # 提取边界点
    up_data, down_data = getUpAndDownPointerList(pred)
    # plt.subplot(1, 2, 1)
    ...
    """上边界处理"""
    # 去除噪声
    up_data = deleteErrorPointer(up_data, k=5, d=15)
    # 绘制去噪后的点
    # for point in up_data:
    #     x, y = point
    #     cv.circle(img, (int(x), int(-y)), 1, (0, 255, 0), -1)
    # cv.imshow('Upper Points After Noise Removal', img)
    # 拟合上边界
    p1_up, up_x, up_y = fitEquation(up_data)
    for i in range(len(up_x)):
        cv.circle(img, (int(up_x[i]), int(-up_y[i])), 2, (0, 255, 0), -1)
    # plt.imshow(img)
    # plt.title("Upper Boundary Fitted")
    # plt.show()

    """下边界处理"""
    # 去除噪声
    down_data = deleteErrorPointer(down_data)
    # 拟合下边界
    p1_down, down_x, down_y = fitEquation(down_data)
    for i in range(len(down_x)):
        cv.circle(img, (int(down_x[i]), int(-down_y[i])), 2, (0, 255, 0), -1)
    # plt.imshow(img)
    # plt.title("Lower Boundary Fitted")
    # plt.show()
    ...
    # for point in up_data:
    #     x, y = point
    #     cv.circle(img_with_boundary, (int(x), int(-y)), 1, (0, 255, 0), -1)  # 绿色点
    # for i in range(len(up_x)):
    #     cv.circle(img_with_boundary, (int(up_x[i]), int(-up_y[i])), 2, (0, 255, 0), -1)  # 绿色拟合曲线

    # 绘制下边界点及拟合曲线
    # for point in down_data:
    #     x, y = point
    #     cv.circle(img_with_boundary, (int(x), int(-y)), 1, (0, 0, 255), -1)  # 红色点
    # for i in range(len(down_x)):
    #     cv.circle(img_with_boundary, (int(down_x[i]), int(-down_y[i])), 2, (0, 0, 255), -1)  # 红色拟合曲线
    #
    # # 同时显示所有结果
    # cv.imshow('原始图像', img)
    # cv.imshow('分割结果', (pred * 255).astype(np.uint8))  # 假设 pred 是 0/1 的掩膜
    # cv.imshow('边界点及拟合曲线', img_with_boundary)
    #
    # # 等待按键后关闭所有窗口
    # cv.waitKey(0)
