...
###################################
# 1. RetinaProcess 类：加载并运行 U-Net，并提供BM层数据生成和圆拟合相关功能
###################################
class RetinaProcess:
    ...
    def process_image(self, bscan_image_path, circle_params):
        """
        完整处理流程：
          1. 对输入B-scan图像进行推理，生成一维结果数组；
          2. 将1D数组转换为 (N, 2) 的BM层点集；
          3. 根据指定参数生成候选圆集合；
          4. 搜索拟合BM层点集得分最佳的圆；
          5. 可视化BM层和最佳拟合圆。

        :param bscan_image_path: B-scan图像的文件路径
        :param circle_params: 候选圆参数配置字典，例如：
               {
        ...
        if best_circle:
            ...
            RetinaProcess.plot_bm_layer_and_circle(bm_layer, bscan_image, best_circle)
        else:
...
if __name__ == '__main__':
    ...
    # 调用封装好的处理流程
    retina_processor.process_image(bscan_image_path, circle_params)
