...
class LensInterface(ScrollArea):
    ...
    def __initHeader(self):
        ...
        # 添加描述
        self.descriptionLabel = BodyLabel("提取晶状体边界，进行曲线拟合并生成分析报告", self.headerWidget)
    ...
    def select_image(self):
        ...
        if file_dialog.exec_():
            ...
            # 在图片卡片中显示图片
            self.originalCard.pixmap = self.original_pixmap
    ...
    def on_process_finished(self, result_data):
        ...
        self.processedCard.pixmap = self.processed_pixmap
        ...
        data_text += "📈 曲线拟合方程\n\n"
        data_text += f"🔷 上边界拟合方程：\n   y = {up_equation.coefficients[0]:.4f}*x² + {up_equation.coefficients[1]:.4f}*x + {up_equation.coefficients[2]:.4f}\n\n"
        data_text += f"🔷 下边界拟合方程：\n   y = {down_equation.coefficients[0]:.4f}*x² + {down_equation.coefficients[1]:.4f}*x + {down_equation.coefficients[2]:.4f}\n"
